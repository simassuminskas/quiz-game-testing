...
module.exports = {
    ...
    sendToAll: function(clients, data, auxClients = null)
    {
        for(var client in auxClients)
        {
            if (auxClients[client].role > 1 && (data.info === 'connection' || data.info === 'disconnection'))
            {
                data.user.ip = module.exports.getUserByID(auxClients, data.user.id).ip;
            }
            auxClients[client].con.write(JSON.stringify(data));
        }
        for (var client in clients)
        {
            if ((!(clients[client].role > 1 && (data.info === 'connection' || data.info === 'disconnection'))) && data.user)
            {
                delete data.user.ip;
            }
        }
