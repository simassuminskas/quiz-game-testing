...
socket.on("update", (data) => {
  ...
  if (
    ...
    data["userSurname"] != undefined &&
    document.getElementById("divGameFinished").style.display == "none"
  ) {
    ...
    if (
      (data["userName"] == userName && data["userSurname"] == userSurname) ||
...
socket.on("showArea1PartialResult", (data) => {
  //console.log(data);
  if (
    data["roomCode"] == roomCode &&
    document.getElementById("divGameFinished").style.display == "none"
    ...
    for (var i = 0; i < teams.length; i++) {
      if (
        teams[i]["teamName"] == data["teamName"] &&
        data["teamName"] == teamName &&
        data["area"] == 1
      ) {
        document.getElementById("area3").style.display = "none";
        ...
        for (var j = 0; j < data["question"]["options"].length; j++) {
          ...
          html +=
            '<div class="optionNoSelected" id="lbl_question_option_' +
            j +
            '">' +
            data["question"]["options"][j]["option"] +
            "<br></div><br>";
        }
...
socket.on("showResultArea2", (data) => {
  ...
  if (
    ...
    for (var j = 0; j < teams.length; j++) {
      if (
        teams[j]["teamName"] == data["teamName"] &&
        data["teamName"] == teamName
      ) {
  ...
  if (
    ...
    for (var j = 0; j < teams.length; j++) {
      if (
        ...
        document.getElementById("area" + data["area"]).style.top =
          parseInt(document.getElementById("lblArea").offsetTop) + 35 + "px";
        ...
        if (data["area"] == 1) {
          ...
          document.getElementById("area1QuestionColumn").innerHTML =
            '<label id="question">' + data["question"]["question"] + "</label>";
          var html = "";
          ...
          document.getElementById("lblLightBoxArea1Header").innerHTML =
            "READ THE DILEMMA & CHOOSE THE BEST ANSWER INDIVIDUALLY.";
        ...
        if (data["area"] == 2) {
          ...
          if (
            ...
            //alert();
            document.getElementById("area2Info").innerHTML =
              "<br><br>PLEASE ANSWER THE QUESTION ABOUT CGI<br> BY CHOOSING 1 OF 3 POSSIBLE ANSWERS";
...
socket.on("area2Question", (data) => {
  if (
    ...
    document.getElementById("area2QuestionsDiv").innerHTML =
    ...
    case "detailedExplanationOfAnswers":
      ...
      for (var i = 0; i < options.length; i++) {
        if (options[i]["option"] == finalAnswer) {
          html +=
            '<div class="lblOptionFinal" id="lbl_question_option_' +
            i +
            '">' +
            options[i]["option"] +
            "<br>" +
            ...
            options[i]["response"] +
            "</label>" +
            "<br>(final answer)<br></div>";
        } else {
          html +=
            '<div class="lblOption" id="lbl_question_option_' +
            i +
            '">' +
            options[i]["option"] +
            "<br>" +
      ...
      if ("no mutual agreement" == finalAnswer) {
        html +=
          '<div class="lblOptionFinal" id="lbl_question_option_' +
          options.length +
          '">no mutual agreement<br>' +
          '<label class="lblScore">-600</label><br>(final answer)<br></div>';
      } else {
        html +=
          '<div class="lblOption" id="lbl_question_option_' +
          options.length +
          '">no mutual agreement<br>' +
          '<label class="lblScore">-600</label><br></div>';
      }
      ...
      document.getElementById("nextBtnDivArea1").innerHTML =
        '<i class="fas fa-angle-right fa-2x" onclick="showNextStep();"></i>';
...
function showNextStep() {
  ...
  switch (nextStep) {
    ...
    case "personalEvaluation": //console.log('Line 297.');
      ...
      document.getElementById("personalEvaluation").innerHTML =
        "EVALUATION<br>ARE WE ACTING LIKE THIS TODAY?<br> " +
        ' Compare to the highest scoring response<br>1 = No, not at all<br>5 = Yes, definitely<br><br>1 <input type="range" id="personalEvaluationRange" min="0" max="4"> 5';
      beforeStep = "showFinalAnswer";
      document.getElementById("beforeBtnDivArea1").innerHTML =
        '<i class="fas fa-angle-left fa-2x" onclick="showBeforeStep();"></i>';
    ...
    case "sendPersonalEvaluation":
      ...
      document.getElementById("beforeBtnDivArea1").style.display = "none";
      var evaluation =
        parseInt(document.getElementById("personalEvaluationRange").value) + 1;
      document.getElementById("personalEvaluation").innerHTML =
        "THANK FOR YOUR ENGAGEMENT<br>PLEASE TAKE TURNS IN SPINING A WHELL";
      socket.emit(
        "personalEvaluation",
        JSON.stringify({
          userName: userName,
    ...
    for (var i = 0; i < data["options"].length; i++) {
      if (data["options"][i]["option"] == finalAnswer) {
        html +=
          ...
          "<br>" +
          '<label class="lblScore">' +
          data["options"][i]["score"] +
          "</label>" +
          " " +
          '<label class="lblResponse">' +
          data["options"][i]["response"] +
          "</label>" +
          "<br>(final answer)<br></div>";
      } else {
        html +=
...
socket.on("leaderVotation", (data) => {
  if (
    ...
    if (teamIndex != -1 && data["teamName"] == teamName) {
      ...
      document.getElementById("lblLightBoxArea1Header").innerHTML =
        " AFTER A GROUP DISCUSSION PLEASE AGREE ON A COMMON ANSWER<br> THE LEADER MUST SUBMIT THE ANSWER";
      ...
      for (var j = 0; j < data["question"]["options"].length; j++) {
        if (leader) {
          ...
          html +=
            '<label class="lblOption" id="lbl_question_option_' +
            j +
            '">' +
            data["question"]["options"][j]["option"] +
            "</label><br><br>";
...
socket.on("personalEvaluation", (data) => {
  if (
    ...
    if (data["teamName"] == teamName) {
      var html =
        'Distant <input type="range" id="personalEvaluationRange" min="0" max="4"> Firm';
      nextStep = "personalEvaluation";
      document.getElementById("nextBtnDivArea1").innerHTML =
        '<i class="fas fa-angle-right fa-2x" onclick="showNextStep();"></i>';
      beforeStep = "showFinalAnswer";
      ...
      document.getElementById("lblLightBoxArea1Header").innerHTML =
        "EVALUATION<br>ARE WE ACTING LIKE THIS TODAY?<br>(Compare to the highest scoring response)";
      document.getElementById("area1Table").style.display = "none";
      document.getElementById("area1LabelsTable").style.display = "none";
...
socket.on("ro", (data) => {
  if (
    ...
    if (data["teamName"] == teamName) {
      document.getElementById("lblWheelInfo").innerHTML = "";
      document.getElementById("spinner").style.display = "none";
      document.getElementById("area3").style.top =
        parseInt(document.getElementById("lblArea").offsetTop) + 35 + "px";
      ...
      if (data["userName"] == userName && data["userSurname"] == userSurname) {
        ...
        document.getElementById("front").innerHTML =
          "CLICK ON THE CARD<br>TO TRY YOUR LUCK!";
        flip("front");
        dataUserName = data["userName"];
        dataUserSurname = data["userSurname"];
      } else {
        ...
        document.getElementById("front").innerHTML =
          ...
          " " +
          data["userSurname"] +
          " WILL OPEN THE CARD<br>TO TRY THE LUCK!";
      }
      showGameInfo();
    }
  }
...
function voteLeader(
  ...
  if (
    document.getElementById("vl_" + userIndex).innerHTML.toLowerCase() !=
    "vote for leader"
  ) {
