...
function showInfo(elementId, eventName) {
  ...
  if (eventName == "onmouseover") {
    if (
      (elementId == "circle" || elementId == "circleText") &&
      !mouseOverCircle &&
      !lockWheel
    ) {
      var top =
        document.getElementById("spinner").offsetTop +
        document.getElementById("spinner").offsetWidth / 2;
      var left =
        document.getElementById("spinner").offsetLeft +
        document.getElementById("spinner").offsetHeight / 2;
      var text = "CLICK TO SPIN THE WHEEL";
      $("#tool-tips").append(
        '<div class="overInfo" id="circleInfo" style="position: absolute; top: ' +
          top +
          "px; z-index: 5; left: " +
          left +
          'px;">' +
    ...
    if (elementId == "spinner" && !mouseOverSpinner && !lockWheel) {
      if (pickedArea != undefined) {
        ...
        var left =
          document.getElementById("spinner").offsetLeft +
          document.getElementById("spinner").offsetHeight;
        var text =
          'CLICK TO OPEN "' +
          ["DILEMMAS", "KNOWLEDGE ABOUT US", "RISKS & OPPORTUNITIES"][
            pickedArea - 1
          ] +
          '"';
        $("#tool-tips").append(
    ...
    if (elementId == "area1AnswersColumn" && !mouseOverArea1AnswersColumn) {
      var top =
        document.getElementById("area1AnswersColumn").offsetTop +
        document.getElementById("area1AnswersColumn").offsetWidth / 2;
      var left =
        document.getElementById("area1AnswersColumn").offsetLeft +
        document.getElementById("area1AnswersColumn").offsetHeight;
      var text =
        "CLICK TO VOTE ON THE ANSWER AND CLICK THE ARROW FOR THE NEXT STEP";
      $("#tool-tips").append(
        '<div class="overInfo" id="area1AnswersColumnInfo" style="position: absolute; top: ' +
          top +
          "px; z-index: 5; left: " +
          left +
          'px;">' +
